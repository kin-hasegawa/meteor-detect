...
当初 `youytube_dl` を使っていたが、YoutTube側の仕様変更に対応するメンテナンスされていないので、`yt-dlp` を使うようにしました。`yt-dlp`は時々変更になるYouTubeのAPIに対応するために、最新にしておく必要がある。
...
その他、地上の建物に光が当たったり、電線が揺れた場合でもノイズを拾うことがあるので環境に合わせてマスクをかけた方が誤認識は減るはずである。
